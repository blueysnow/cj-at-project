...
namespace TradeLink.Common
{
    ...
    public static class MyCalc
    {
        ...
        public static decimal[] AbsoluteReturn(PositionTracker pt, decimal[] marketprices, bool countClosedPL, bool countOpenPL)
        {
            ...
            {
                if (countOpenPL)
                    aret[i] += MyCalc.OpenPL(marketprices[i], pt[i]);
                if (countClosedPL)
        ...
        public static decimal[] AbsoluteReturn(PositionTracker pt, GenericTracker<decimal> marketprices, bool countClosedPL)
        {
            ...
            for (int i = 0; i < pt.Count; i++)
            {
                ...
                if (countOpenPL && (idx >= 0))
                    aret[i] += MyCalc.OpenPL(marketprices[idx], p);
                if (countClosedPL)
                    aret[i] += p.ClosedPL;
            }
            return aret;
        ...
        public static decimal MaxDDPct(List<Trade> fills)
        {
            ...
            {
                pt.Adjust(fills[i]);
                decimal miu = MyCalc.Sum(MyCalc.MoneyInUse(pt));
                if (miu > mmiu)
                    mmiu = miu;
                ret.Add(MyCalc.Sum(MyCalc.AbsoluteReturn(pt, new decimal[0], true, false)));
            }
